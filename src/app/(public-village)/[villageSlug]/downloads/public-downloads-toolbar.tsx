...
function href(villageSlug: string, keyword: string, sort: SortValue) {
  ...
  return query ? `/${villageSlug}/downloads?${query}` : `/${villageSlug}/downloads`;
}

export function PublicDownloadsToolbar({ villageSlug, keyword, sort, suggestionTitles }: Props) {
  return <PublicPageToolbar namespace="public-downloads" keyword={keyword} placeholder="ค้นหาเอกสาร" suggestions={suggestionTitles} activeFilterCount={Number(sort !== "newest")} filters={<ResidentFilterDropdown label="เรียง" options={[
