...
export function FirstSuperAdminSetupForm() {
  ...
  return <form action={formAction} className="mt-6 space-y-4">
    ...
    <Input label="เบอร์โทรศัพท์" name="phoneNumber" type="tel" inputMode="numeric" placeholder="0812345678" autoComplete="tel" pattern="[0-9]{10}" maxLength={10} value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value.replace(/\D/g, "").slice(0, 10))} required />
