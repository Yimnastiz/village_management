...
export default async function ResidentContactRequestDetailPage({ params }: PageProps) {
  ...
  const fields = [
    ...
    ["ตำแหน่ง", value(request.role)],
    ["อีเมล", value(request.email)],
    ["ที่อยู่", value(request.address)],
    ["หมวดหมู่", value(request.category)],
  ];
  ...
  return <div className="mx-auto w-full max-w-3xl space-y-6">
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2"><h1 className="text-xl font-semibold text-gray-900">คำขอเพิ่มผู้ติดต่อ</h1><Badge variant={statusVariant[request.status]}>{statusCopy[request.status]}</Badge></div>
      <p className="mt-1 text-sm text-gray-500">ส่งเมื่อ {request.createdAt.toLocaleString("th-TH")}</p>
      <dl className="mt-5 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">{fields.map(([label, fieldValue]) => <div key={label}><dt className="text-gray-500">{label}</dt><dd className="font-medium text-gray-900">{fieldValue}</dd></div>)}</dl>
      ...
      {request.status === "APPROVED" && request.approvedContactId ? <Link href={`/resident/contacts/${request.approvedContactId}`} className="mt-5 inline-flex rounded-md border border-green-300 px-3 py-1.5 text-sm font-medium text-green-700 hover:bg-green-50">ดูข้อมูลผู้ติดต่อที่อนุมัติแล้ว</Link> : null}
