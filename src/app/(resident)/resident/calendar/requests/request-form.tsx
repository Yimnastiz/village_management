...
import { z } from "zod";
import { RequestedVisibilityField } from "@/components/resident/requested-visibility-field";
...
const schema = z.object({
  ...
  startsAt: z.string().min(1, "กรุณาระบุวันที่และเวลาเริ่ม"),
  ...
  if (Number.isNaN(endsAt.getTime()) || endsAt < startsAt) context.addIssue({ code: "custom", path: ["endsAt"], message: "วันที่และเวลาสิ้นสุดต้องไม่ก่อนวันที่และเวลาเริ่ม" });
});
...
export function CalendarRequestForm({ requestId, initialValues, approved = false, embedded = false, formId, hideActions = false, onSuccess, onCancel, onSubmittingChange }: CalendarRequestFormProps = {}) {
  ...
  const { register, handleSubmit, setError, clearErrors, reset, formState: { errors, isSubmitting } } = useForm<CalendarRequestFormData>({
    ...
    defaultValues: { title: initialValues?.title ?? "", description: initialValues?.description ?? "", location: initialValues?.location ?? "", startsAt: initialValues?.startsAt ?? "", endsAt: initialValues?.endsAt ?? "", ...(initialValues?.visibility ? { visibility: initialValues.visibility } : {}) },
  });
  ...
  return <form id={formId} onSubmit={handleSubmit(onSubmit)} className={embedded ? "space-y-4" : "space-y-4 rounded-xl border border-gray-200 bg-white p-4 sm:p-6"}>
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Input label="วันที่และเวลาเริ่ม" type="datetime-local" required {...register("startsAt")} error={errors.startsAt?.message} /><Input label="วันที่และเวลาสิ้นสุด" type="datetime-local" {...register("endsAt")} error={errors.endsAt?.message} /></div>
    <Input label="สถานที่" {...register("location")} error={errors.location?.message} />
    <RequestedVisibilityField publicInput={register("visibility")} residentInput={register("visibility")} residentValue="RESIDENT" helperId="calendar-request-visibility-helper" error={errors.visibility?.message} />
    {errors.root ? <p className="text-sm text-red-600">{errors.root.message}</p> : null}
    {approved ? <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">รายการนี้ได้รับอนุมัติแล้ว การแก้ไขจะเป็นคำขอแก้ไขที่รอผู้ดูแลหมู่บ้านอนุมัติ</p> : null}
