...
export function ResidentEventRequestModal({ triggerLabel = "ขอเพิ่มกิจกรรม" }: { triggerLabel?: string }) {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const formId = "resident-calendar-event-request";
  return <>
    <Button type="button" size="sm" className="h-10 px-2 sm:px-3" onClick={() => setOpen(true)}><FilePlus2 className="h-4 w-4" /><span className="ml-1 hidden min-[390px]:inline">{triggerLabel}</span></Button>
