...
type Props = { mode?: "create" | "update"; targetPlaceId?: string; defaultValues?: FormData & { images: PlaceImageView[] } };

export function PlaceRequestForm({ mode = "create", targetPlaceId, defaultValues }: Props) {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
    ...
    <div className="flex flex-col-reverse gap-3 sm:flex-row"><Button type="button" variant="outline" onClick={() => router.back()}>ย้อนกลับ</Button><Button type="submit" disabled={uploadsBusy || isSubmitting} isLoading={isSubmitting}>{mode === "update" ? "ส่งคำขอแก้ไขสถานที่" : "ส่งคำขอเพิ่มสถานที่"}</Button></div>
  </form>;
}
