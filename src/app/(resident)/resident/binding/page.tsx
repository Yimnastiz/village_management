...
export default async function ResidentBindingPage() {
  ...
  const latestRequest = session
    ? await prisma.bindingRequest.findFirst({
        where: { userId: session.id, villageId: village.id, status: { not: BindingRequestStatus.CANCELLED } },
  ...
  const houses = await prisma.house.findMany({
    ...
    select: { id: true, houseNumber: true, normalizedHouseNumber: true },
  });
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
        <BindingRequestForm
          village={village}
          houses={houses}
          latestRequest={latestRequest ? { houseId: latestRequest.houseId, houseNumber: latestRequest.houseNumber, note: latestRequest.note } : null}
