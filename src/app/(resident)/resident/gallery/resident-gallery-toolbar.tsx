...
interface ResidentGalleryToolbarProps {
  keyword: string;
  sort: SortValue;
  villageName: string;
...
function buildGalleryHref(params: {
  ...
  if (params.allowSubmissionsOnly) {
    query.set("allowSubmissions", "1");
  }
  ...
  return queryString ? `/resident/gallery?${queryString}` : "/resident/gallery";
}

export function ResidentGalleryToolbar({
  keyword,
  sort,
  villageName,
  ...
  }, [searchKeyword, searchOpen, sort, selectedVisibilities, allowSubmissionsOnly, router]);
  ...
  const toggleSubmissionsHref = buildGalleryHref({
    keyword: searchKeyword,
    sort,
    visibilities: selectedVisibilities,
