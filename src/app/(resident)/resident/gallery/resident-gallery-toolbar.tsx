...
export function ResidentGalleryToolbar({ keyword, sort, villageName, selectedVisibilities, allowSubmissionsOnly, suggestionTitles, canSubmit, hasResidentAccess }: Props) {
  ...
  const activeFilterCount = hasResidentAccess ? selectedVisibilities.length + Number(allowSubmissionsOnly) : 0;
  return <ResidentPageToolbar namespace="resident-gallery" title="แกลเลอรี" registerHeader search={{ keyword, placeholder: "พิมพ์ชื่ออัลบั้ม", label: "ค้นหาอัลบั้ม", suggestions: suggestionTitles }} activeFilterCount={activeFilterCount} actions={canSubmit ? <Link href="/resident/gallery/requests" className="inline-flex min-h-11 items-center rounded-lg border border-gray-300 bg-white px-3 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">คำขอของฉัน</Link> : null} filters={<>{hasResidentAccess ? <><span className="text-xs font-semibold text-gray-500">การมองเห็น</span>{([['PUBLIC', 'สาธารณะ'], ['RESIDENT_ONLY', 'เฉพาะลูกบ้าน']] as const).map(([value, label]) => <NewsFilterChip key={value} href={toggle(value)} active={has(value)}>{label}</NewsFilterChip>)}<NewsFilterChip href={makeHref(keyword, sort, selectedVisibilities, !allowSubmissionsOnly, hasResidentAccess)} active={allowSubmissionsOnly}>ส่งรูปเพิ่มได้</NewsFilterChip></> : null}<span className="ml-1 text-xs font-semibold text-gray-500">เรียง</span>{([['newest', 'ล่าสุดก่อน'], ['oldest', 'เก่าก่อน']] as const).map(([value, label]) => <NewsFilterChip key={value} href={makeHref(keyword, value, selectedVisibilities, allowSubmissionsOnly, hasResidentAccess)} active={sort === value}>{label}</NewsFilterChip>)}{activeFilterCount ? <NewsFilterChip href={makeHref(keyword, "newest", [], false, hasResidentAccess)} active={false}>ล้างตัวกรอง</NewsFilterChip> : null}</>} />;
}
