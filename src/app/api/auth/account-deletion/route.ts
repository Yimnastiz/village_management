...
export async function DELETE(request: NextRequest) {
  ...
  const user = await prisma.user.findFirst({ where: { deletionRecoveryHash: hashRecoveryToken(token), accountStatus: AccountStatus.DELETION_PENDING, scheduledDeletionAt: { gt: new Date() } }, select: { id: true, memberships: { select: { villageId: true, role: true } } } });
  ...
  await prisma.$transaction([
    ...
    prisma.auditLog.createMany({ data: (user.memberships.length ? user.memberships : [{ villageId: null, role: null }]).map((membership) => ({ userId: user.id, villageId: membership.villageId, action: AuditAction.UPDATE, resource: "UserAccount", resourceId: user.id, metadata: { actorRole: membership.role, actionName: "ACCOUNT_DELETION_CANCELLED", status: AccountStatus.ACTIVE, deletionCancelled: true } })) }),
  ]);
