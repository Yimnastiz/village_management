...
export default async function VillageBindingRequestsPage({ params }: { params: Promise<{ villageId: string }> }) {
  ...
  const rows = await prisma.bindingRequest.findMany({ where: { villageId }, orderBy: { createdAt: "desc" }, take: 50, select: { id: true, status: true, houseId: true, houseNumber: true, reviewNote: true, createdAt: true, reviewedAt: true, reviewedBy: true, user: { select: { name: true } } } });
