...
const labels: Record<HouseholdOccupancyStatus,string>={OCCUPIED:"มีผู้อยู่อาศัย",VACANT:"ว่าง",UNDER_CONSTRUCTION:"กำลังก่อสร้าง",DEMOLISHED:"รื้อถอนแล้ว"};
export default async function Page({params,searchParams}:{params:Promise<{villageId:string}>;searchParams:Promise<{q?:string;status?:string;page?:string}>}){
 ...
 const q=(search.q??"").trim(); const status=Object.values(HouseholdOccupancyStatus).includes(search.status as HouseholdOccupancyStatus)?search.status as HouseholdOccupancyStatus:null; const page=Math.max(1,Number(search.page)||1); const size=25;
 const where:Prisma.HouseWhereInput={villageId,...(q?{OR:[{houseNumber:{contains:q,mode:"insensitive"}},{address:{contains:q,mode:"insensitive"}}]}:{}),...(status?{occupancyStatus:status}:{})};
 ...
 <form method="get" className="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-[1fr_220px_auto]"><input name="q" defaultValue={q} placeholder="ค้นหาเลขที่บ้านหรือที่อยู่" className="min-h-10 rounded-lg border border-slate-300 px-3 text-sm"/><select name="status" defaultValue={status??"ALL"} className="min-h-10 rounded-lg border border-slate-300 px-3 text-sm"><option value="ALL">ทุกสถานะ</option>{Object.entries(labels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><button className="min-h-10 rounded-lg bg-slate-900 px-4 text-sm font-medium text-white">ค้นหา</button></form>
 <section className="overflow-hidden rounded-xl border bg-white"><div className="overflow-x-auto"><table className="min-w-[720px] w-full text-sm"><thead className="bg-slate-50 text-left text-slate-500"><tr><th className="px-4 py-3">บ้านเลขที่</th><th className="px-4 py-3">สถานะ</th><th className="px-4 py-3">ประชากร</th><th className="px-4 py-3">สมาชิกระบบ</th><th className="px-4 py-3 text-right">จัดการ</th></tr></thead><tbody>{rows.map(row=><tr key={row.id} className="border-t"><td className="px-4 py-3 font-medium">{row.houseNumber}<p className="mt-0.5 text-xs font-normal text-slate-400">{row.address||"ไม่ระบุที่อยู่เพิ่มเติม"}</p></td><td className="px-4 py-3"><Badge variant="outline">{labels[row.occupancyStatus]}</Badge></td><td className="px-4 py-3">{row._count.persons} คน</td><td className="px-4 py-3">{row._count.memberships} บัญชี</td><td className="px-4 py-3 text-right"><Link href={`${base}/${row.id}`} className="inline-flex min-h-9 items-center rounded-lg border px-3 font-medium hover:bg-slate-50">ดูรายละเอียด</Link></td></tr>)}</tbody></table></div>{!rows.length?<div className="border-t p-10 text-center text-sm text-slate-500">ไม่พบข้อมูลบ้านตามเงื่อนไข</div>:null}</section>
 <QueryPagination pathname={base} page={page} totalPages={Math.max(1,Math.ceil(total/size))} params={{q:q||undefined,status:status??undefined}}/></div>;
}
