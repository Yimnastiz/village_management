...
export default async function SuperAdminVillagesPage({ searchParams }: PageProps) {
  ...
  const [villages, totalCount] = hasSearch
    ? await Promise.all([
        prisma.village.findMany({ where, orderBy: [{ isActive: "desc" }, { name: "asc" }], skip: (page - 1) * pageSize, take: pageSize, include: { _count: { select: { memberships: true, houses: true, news: true } } } }),
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        {!hasSearch ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center"><h2 className="text-base font-semibold text-slate-900">ค้นหาหมู่บ้านเพื่อเริ่มจัดการ</h2><p className="mx-auto mt-2 max-w-xl text-sm text-slate-500">พิมพ์ชื่อหมู่บ้าน ตำบล อำเภอ หรือจังหวัด เพื่อค้นหาหมู่บ้านที่ต้องการจัดการ</p></div> : null}
        {hasSearch && villages.length === 0 ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center"><h2 className="text-base font-semibold text-slate-900">ไม่พบหมู่บ้านที่ตรงกับคำค้นหา</h2><p className="mt-2 text-sm text-slate-500">ลองเปลี่ยนคำค้นหา หรือเพิ่มหมู่บ้านใหม่จากปุ่มด้านบน</p></div> : null}
      ...
      {hasSearch ? <QueryPagination pathname="/superadmin/villages" page={page} totalPages={totalPages} params={{ q: keyword, status: status !== "all" ? status : undefined, province: province || undefined }} /> : null}
    </div>
  );
}
