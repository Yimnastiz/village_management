...
export default async function Page({ params, searchParams }: PageProps) {
  ...
  const [houses, total, houseSuggestions] = await Promise.all([
    prisma.house.findMany({ where, include: { _count: { select: { persons: { where: { status: PersonStatus.ACTIVE } }, memberships: { where: { status: MembershipStatus.ACTIVE } } } } }, orderBy: [{ normalizedHouseNumber: sort }], take: 300 }),
  ...
  return <div className="flex min-h-0 flex-col gap-3 sm:h-[calc(100dvh-var(--app-topbar-visible-offset,4rem)-2rem)] sm:overflow-hidden">
