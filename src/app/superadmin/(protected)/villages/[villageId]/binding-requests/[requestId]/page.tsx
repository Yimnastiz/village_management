...
export default async function Page({ params }: { params: Promise<{ villageId: string; requestId: string }> }) {
  ...
  const request = await prisma.bindingRequest.findFirst({ where: { id: requestId, villageId }, include: { village: { select: { name: true } }, house: { select: { id: true, villageId: true, houseNumber: true, persons: { where: { villageId }, select: { id: true, firstName: true, lastName: true, nationalId: true, status: true } } } }, user: { select: { id: true, name: true, phoneNumber: true, person: { select: { nationalId: true, villageId: true } } } } } });
  if (!request || (request.house && request.house.villageId !== villageId)) notFound();
  const houses = await prisma.house.findMany({ where: { villageId }, select: { id: true, houseNumber: true }, orderBy: { houseNumber: "asc" } });
  const nationalId = request.user.person?.villageId === villageId ? request.user.person.nationalId : null;
  const duplicate = nationalId ? await prisma.person.count({ where: { villageId, nationalId, userId: { not: request.user.id } } }) : 0;
