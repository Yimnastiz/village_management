...
import { BINDING_REQUEST_STATUS_LABELS as BASE_BINDING_REQUEST_STATUS_LABELS, MEMBERSHIP_ROLE_LABELS } from "@/lib/constants";
import { getWorkspaceVillage } from "@/features/village-workspace/server/queries";
import { prisma } from "@/lib/prisma";
import { requireSuperAdminPageSession } from "@/lib/superadmin";
import { WorkspaceListPage, WorkspaceTable } from "../workspace-list-page";

const historyStatuses = [BindingRequestStatus.APPROVED, BindingRequestStatus.REJECTED] as const;

function bindingStatusLabel(status: BindingRequestStatus) {
  return status === BindingRequestStatus.APPROVED ? "อนุมัติ" : BASE_BINDING_REQUEST_STATUS_LABELS[status];
}
...
export default async function Page({ params, searchParams }: { params: Promise<{ villageId: string }>; searchParams: Promise<{ tab?: string; q?: string; status?: string }> }) {
  ...
  const [pendingCount, total, requests] = await Promise.all([
    ...
    prisma.bindingRequest.findMany({ where, orderBy: { createdAt: tab === "pending" ? "asc" : "desc" }, include: { user: { select: { name: true, phoneNumber: true } }, house: { select: { houseNumber: true, villageId: true } } } }),
  ]);
  ...
  const query = (next: { tab?: typeof tab; q?: string; status?: (typeof historyStatuses)[number] }) => {
  ...
  return <WorkspaceListPage><div className="flex min-h-0 flex-col sm:h-[calc(100dvh-var(--app-topbar-visible-offset,4rem)-2rem)] sm:overflow-visible">
    ...
    <AdminListToolbar sticky hideHeading title="คำขอผูกเลขบ้าน" description={`ตรวจสอบและจัดการคำขอของ ${village.name}`} actions={requestTabs} searchAction={base} clearHref={query({ tab, q: "" })} keyword={keyword} searchLabel="ค้นหาคำขอผูกบ้าน" searchPlaceholder="ค้นหาชื่อ เบอร์โทร หรือบ้านเลขที่" groups={tab === "history" ? [{ label: "สถานะ", options: [
      ...
      ...historyStatuses.map((value) => ({ label: bindingStatusLabel(value), href: query({ tab, q: keyword, status: value }), active: selectedStatus === value })),
    ] }] : []} />
    <WorkspaceTable className="mt-2" empty={!requests.length}>
      {requests.length ? <>
        <div className="divide-y divide-gray-100 md:hidden">{requests.map((request) => { const reviewer = request.reviewedBy ? reviewerById.get(request.reviewedBy) : undefined; return <Link key={request.id} href={`${base}/${request.id}`} className="block p-4 transition-colors hover:bg-blue-50/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-green-500"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold text-gray-900">{request.user.name}</p><p className="mt-1 text-sm text-gray-600">{request.user.phoneNumber}</p></div><Badge variant={statusVariant(request.status)}>{bindingStatusLabel(request.status)}</Badge></div><p className="mt-3 text-sm text-gray-700">บ้านเลขที่ {requestedHouse(request)}</p>{missingHouse(request) ? <p className="mt-2 inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-800">ต้องตรวจสอบบ้านเพิ่ม</p> : null}<p className="mt-3 text-xs text-gray-500">ยื่นเมื่อ {request.createdAt.toLocaleString("th-TH")}</p>{tab === "history" ? <p className="mt-1 text-xs text-gray-500">ผู้พิจารณา {reviewer?.name ?? "-"} ({reviewerRoleLabel(reviewer)}) · {request.reviewedAt?.toLocaleString("th-TH") ?? "-"}</p> : null}</Link>})}</div>
        <div className="hidden min-h-0 flex-1 overflow-auto md:block"><table className="min-w-[900px] w-full text-sm"><thead className="sticky top-0 z-20 bg-gray-50 text-left text-gray-600 shadow-sm"><tr><th scope="col" className="bg-gray-50 px-3 py-3">ผู้ยื่นคำขอ</th><th scope="col" className="bg-gray-50 px-3 py-3">เบอร์โทร</th><th scope="col" className="bg-gray-50 px-3 py-3">บ้านที่ขอ</th><th scope="col" className="whitespace-nowrap bg-gray-50 px-3 py-3">วันที่ยื่น</th>{tab === "history" ? <><th scope="col" className="whitespace-nowrap bg-gray-50 px-3 py-3">ผู้พิจารณา</th><th scope="col" className="whitespace-nowrap bg-gray-50 px-3 py-3">วันที่พิจารณา</th></> : null}<th scope="col" className="whitespace-nowrap bg-gray-50 px-3 py-3">สถานะ</th><th scope="col" className="whitespace-nowrap bg-gray-50 px-3 py-3">การจัดการ</th></tr></thead><tbody>{requests.map((request) => { const reviewer = request.reviewedBy ? reviewerById.get(request.reviewedBy) : undefined; return <tr key={request.id} className="border-t border-gray-100 transition-colors hover:bg-blue-50/60 focus-within:bg-blue-50/60"><td className="px-3 py-3 font-medium text-gray-900">{request.user.name}</td><td className="px-3 py-3 text-gray-700">{request.user.phoneNumber}</td><td className="px-3 py-3 text-gray-700"><p>{requestedHouse(request)}</p>{missingHouse(request) ? <span className="mt-1 inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-800">ต้องตรวจสอบบ้านเพิ่ม</span> : null}</td><td className="whitespace-nowrap px-3 py-3 text-gray-700">{request.createdAt.toLocaleString("th-TH")}</td>{tab === "history" ? <><td className="whitespace-nowrap px-3 py-3 text-gray-700">{reviewer?.name ?? "-"}</td><td className="whitespace-nowrap px-3 py-3 text-gray-700">{request.reviewedAt?.toLocaleString("th-TH") ?? "-"}</td></> : null}<td className="px-3 py-3"><Badge variant={statusVariant(request.status)}>{bindingStatusLabel(request.status)}</Badge></td><td className="px-3 py-3"><Link href={`${base}/${request.id}`} className="inline-flex min-h-9 whitespace-nowrap items-center justify-center rounded-lg border border-gray-300 px-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2">ดู</Link></td></tr>})}</tbody></table></div>
