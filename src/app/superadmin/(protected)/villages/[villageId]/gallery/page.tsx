...
export default async function SuperAdminGalleryPage({ params, searchParams }: { params: Promise<{ villageId: string }>; searchParams?: Promise<{ q?: string; visibility?: string; sort?: string }> }) {
  ...
  return <WorkspaceListPage><div className="flex min-h-0 flex-col gap-3">
    <SuperAdminPageHeaderRegistration priority={1} context={{ title: "แกลเลอรี", description: `จัดการอัลบั้มและรูปภาพของ ${village?.name ?? "หมู่บ้าน"} เพื่อการสนับสนุนงานหมู่บ้าน` }} />
    <AdminListToolbar sticky hideHeading title="แกลเลอรี" description={`จัดการอัลบั้มและรูปภาพของ ${village?.name ?? "หมู่บ้าน"} เพื่อการสนับสนุนงานหมู่บ้าน`} searchAction={base} clearHref={base} keyword={keyword} searchPlaceholder="ค้นหาชื่ออัลบั้มหรือคำอธิบาย" hiddenInputs={{ visibility: visibility === "ALL" ? "" : visibility, sort: query.sort === "oldest" ? "oldest" : "" }} groups={[{ label: "การมองเห็น", options: [{ label: "ทั้งหมด", href: base, active: visibility === "ALL" }, { label: "สาธารณะ", href: `${base}?visibility=PUBLIC`, active: visibility === "PUBLIC" }, { label: "เฉพาะลูกบ้าน", href: `${base}?visibility=RESIDENT`, active: visibility === "RESIDENT" }] }, { label: "เรียงลำดับ", options: [{ label: "ล่าสุดก่อน", href: base, active: query.sort !== "oldest" }, { label: "เก่าก่อน", href: `${base}?sort=oldest`, active: query.sort === "oldest" }]}]} actions={<><Link href={`${base}/submissions`} className="inline-flex items-center justify-center rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">คำขอเพิ่มรูป {pending ? `(${pending})` : ""}</Link><Link href={`${base}/new`} className="inline-flex items-center justify-center rounded-lg bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700"><Plus className="mr-1 h-4 w-4" />เพิ่มอัลบั้ม</Link></>} />
    {albums.length === 0 ? <div className="rounded-xl border border-dashed border-gray-200 bg-white p-10 text-center"><ImagePlus className="mx-auto mb-3 h-10 w-10 text-gray-300" /><p className="text-gray-600">{keyword || visibility !== "ALL" ? "ไม่พบอัลบั้มที่ตรงกับเงื่อนไข" : "ยังไม่มีอัลบั้มรูปภาพ"}</p></div> : <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">{albums.map((album) => <Link key={album.id} href={`${base}/${album.id}`} className="min-w-0 overflow-hidden rounded-xl border border-gray-200 bg-white transition-shadow hover:shadow-md"><div className="aspect-video bg-gray-100">{album.items[0]?.fileUrl ?? album.coverUrl ? <img src={album.items[0]?.fileUrl ?? album.coverUrl ?? ""} alt={album.title} className="h-full w-full object-cover" loading="lazy" /> : <div className="flex h-full items-center justify-center text-sm text-gray-400">ไม่มีรูปหน้าปก</div>}</div><div className="space-y-2 p-4"><div className="flex flex-wrap gap-2"><Badge variant={album.isPublic ? "success" : "info"}>{album.isPublic ? "สาธารณะ" : "เฉพาะลูกบ้าน"}</Badge>{album.allowResidentSubmissions ? <Badge variant="warning">รับคำขอรูป</Badge> : null}<Badge variant="outline">{album._count.items} รูป</Badge></div><p className="truncate font-medium text-gray-900">{album.title}</p><p className="text-xs text-gray-500">วันที่อัลบั้ม {formatThaiShortDate(album.albumDate)}</p></div></Link>)}</div>}
