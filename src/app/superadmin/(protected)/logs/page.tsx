...
import { auditActionLabel, auditModuleLabel, auditResourcesForModule, formatAuditEvent, IMPORTANT_AUDIT_RESOURCES } from "@/lib/audit-event";
...
const actorRoleLabels: Record<string, string> = { HEADMAN: "ผู้ใหญ่บ้าน", ASSISTANT_HEADMAN: "ผู้ช่วยผู้ใหญ่บ้าน", SUPERADMIN: "ผู้ดูแลระบบระดับสูง", ADMIN: "ผู้ดูแลระบบระดับสูง", USER: "ผู้ใช้งาน" };
...
export default async function SuperAdminLogsPage({ searchParams }: PageProps) {
  ...
  const module = moduleOptions.includes((params.module ?? "").trim()) ? (params.module ?? "").trim() : ""; const dateFrom = (params.dateFrom ?? "").trim(); const dateTo = (params.dateTo ?? "").trim();
  ...
  const startDate = validDate(dateFrom); const endDate = validDate(dateTo, true); const resources = module ? auditResourcesForModule(module) : []; const resourceFilter = resources.length ? (view === "important" ? resources.filter((item) => IMPORTANT_AUDIT_RESOURCES.includes(item as typeof IMPORTANT_AUDIT_RESOURCES[number])) : resources) : view === "important" ? [...IMPORTANT_AUDIT_RESOURCES] : [];
  ...
  const rows: AuditLogRow[] = rawLogs.map((log) => {
    ...
    const formatted = formatAuditEvent({ action: log.action, resource: log.resource, metadata: log.metadata }); const actorRoleValue = typeof metadata.actorRole === "string" ? metadata.actorRole : log.user?.systemRole ?? null; const actorRole = actorRoleValue ? actorRoleLabels[actorRoleValue] ?? actorRoleValue : null; const actor = log.user?.name || log.user?.phoneNumber || "ระบบ";
  ...
  const base = { q: keyword || undefined, view, action: selectedAction ?? undefined, module: module || undefined, dateFrom: startDate ? dateFrom : undefined, dateTo: endDate ? dateTo : undefined };
  ...
  const moduleGroup: ToolbarGroup = { label: "ประเภทข้อมูล", options: [{ label: "ทั้งหมด", href: makeHref(base), active: !module, isDefault: true }, ...moduleOptions.map((item) => ({ label: auditModuleLabel(item), href: makeHref({ ...base, module: item }), active: item === module }))] };
  const viewGroup: ToolbarGroup = { label: "มุมมอง", countsAsFilter: false, options: [{ label: "กิจกรรมทั้งหมด", href: makeHref({ ...base, view: "all" }), active: view === "all", isDefault: true }, { label: "บันทึกสำคัญ", href: makeHref({ ...base, view: "important" }), active: view === "important" }] };
