...
type Props = { initialSettings: SystemSettingsInput };
...
function SwitchRow({ id, title, helper, checked, disabled, onChange }: { id: string; title: string; helper: string; checked: boolean; disabled: boolean; onChange: (value: boolean) => void }) {
  return <div className="flex flex-col gap-3 rounded-xl border border-slate-200 px-4 py-4 sm:flex-row sm:items-start sm:justify-between"><div><label htmlFor={id} className="text-sm font-semibold text-slate-900">{title}</label><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">{helper}</p><p className="mt-2 text-xs font-medium text-slate-500">สถานะ: {checked ? "เปิดใช้งาน" : "ปิดใช้งาน"}</p></div><button id={id} type="button" role="switch" aria-checked={checked} disabled={disabled} onClick={() => onChange(!checked)} className={`relative h-7 w-12 shrink-0 rounded-full transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${checked ? "bg-cyan-600" : "bg-slate-300"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-6" : "translate-x-1"}`} /></button></div>;
}
...
export function SystemSettingsForm({ initialSettings }: Props) {
  ...
  const save = async (next: SystemSettingsInput) => {
    setPending(true);
    try { await updateSystemSettingsAction(next); setSettings(next); success("บันทึกการตั้งค่าระบบแล้ว"); }
