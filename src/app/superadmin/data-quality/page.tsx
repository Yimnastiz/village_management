...
export default async function DataQualityPage() {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-bold">Data Quality</h1><p className="text-sm text-slate-500">รายการตรวจสอบแบบอ่านอย่างเดียว ระบบจะไม่แก้ข้อมูลอัตโนมัติ</p></div><div className="grid gap-4 lg:grid-cols-2">{groups.map(([title,rows])=><section key={title} className="rounded-xl border bg-white p-4"><h2 className="font-semibold">{title} <span className="text-slate-400">({rows.length})</span></h2><div className="mt-2 max-h-64 overflow-auto text-sm text-slate-600">{rows.length===0?<p>ไม่พบปัญหา</p>:rows.map((row,index)=><p key={`${row}-${index}`} className="border-b py-1.5">{row}</p>)}</div></section>)}</div></div>;
}
