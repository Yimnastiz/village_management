...
function DownloadAttachmentEditor({ value, onChange, disabled, error }: { value: Attachment[]; onChange: (next: Attachment[]) => void; disabled: boolean; error?: string }) {
  ...
  return <div className="space-y-3">
    ...
    {value.length === 0 ? <button type="button" disabled={disabled} onClick={() => inputRef.current?.click()} className="flex min-h-28 w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 px-4 text-center text-sm text-gray-600 hover:border-green-500 hover:bg-green-50 disabled:cursor-not-allowed"><UploadCloud className="mb-2 h-7 w-7 text-gray-400" />เพิ่มไฟล์เอกสาร</button> : <ul className="overflow-hidden rounded-xl border border-gray-200 divide-y divide-gray-100">{value.map((item) => <li key={item.localId} className="flex min-w-0 items-center gap-3 px-3 py-3 sm:px-4"><FileText className="h-5 w-5 shrink-0 text-gray-400" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-gray-800">{item.fileName}</p><p className={item.status === "error" ? "mt-0.5 text-xs text-rose-600" : "mt-0.5 text-xs text-gray-500"}>{item.status === "uploading" ? "กำลังอัปโหลด..." : item.status === "error" ? item.error : `${downloadTypeLabel(item.mimeType ?? null, item.fileName)} · ${formatFileSize(item.fileSize ?? 0)}`}</p></div>{item.status === "uploading" ? <LoaderCircle className="h-5 w-5 shrink-0 animate-spin text-green-600" /> : null}{item.id ? <a href={`/api/downloads/${item.id}`} className="hidden min-h-11 items-center px-2 text-sm text-green-700 hover:underline sm:inline-flex">เปิดไฟล์</a> : null}<Button type="button" variant="ghost" size="sm" className="min-h-11 min-w-11 px-2 text-rose-600 hover:bg-rose-50 hover:text-rose-700" disabled={disabled} onClick={() => remove(item.localId)} aria-label={`ลบ ${item.fileName}`}><Trash2 className="h-4 w-4" /></Button></li>)}</ul>}
...
export function DownloadForm({ mode, fileId, defaultValues, initialAttachments = [] }: DownloadFormProps) {
  ...
  const { register, handleSubmit, watch, setError, setValue, formState: { errors, isSubmitting } } = useForm<FormValues>({ defaultValues: { ...(defaultValues ?? { title: "", description: "", category: "", visibility: "PUBLIC" as const }), categoryLabel: defaultValues?.categoryLabel ?? "" } });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const payload: DownloadFormInput = { ...data, attachments: attachments.map(({ localId: _localId, status: _status, error: _error, ...item }) => item) };
