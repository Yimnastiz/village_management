...
export function DownloadManagementActions({ fileId, stage }: { fileId: string; stage: "DRAFT" | "PUBLISHED" | "ARCHIVED" }) {
  ...
  const run = async (reason = "") => {
    if (!dialog) return;
    setPending(true);
    const result = dialog === "publish" ? await publishDownloadAction(fileId) : dialog === "archive" ? await archiveDownloadAction(fileId, reason) : dialog === "restore" ? await restoreDownloadAction(fileId) : await deleteDownloadAction(fileId, reason);
    setPending(false);
