...
export function AuditDetailDialog({ detail, onClose }: { detail: AuditDetail | null; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-end justify-end bg-slate-950/40 sm:items-stretch" role="presentation" onMouseDown={onClose}>
    <section role="dialog" aria-modal="true" aria-labelledby="audit-detail-title" className="max-h-[88vh] w-full overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:h-full sm:max-h-none sm:max-w-md sm:rounded-none" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <dl className="mt-6 space-y-4 text-sm"><div><dt className="text-gray-500">ผู้ดำเนินการ</dt><dd className="mt-1 font-medium text-gray-900">{detail.actor}</dd></div>{detail.actorRole ? <div><dt className="text-gray-500">บทบาท</dt><dd className="mt-1 font-medium text-gray-900">{detail.actorRole}</dd></div> : null}<div><dt className="text-gray-500">เหตุการณ์</dt><dd className="mt-1 font-medium text-gray-900">{detail.event}</dd></div>{detail.item ? <div><dt className="text-gray-500">รายการ</dt><dd className="mt-1 font-medium text-gray-900">{detail.item}</dd></div> : null}{detail.referenceId ? <div><dt className="text-gray-500">รหัสอ้างอิง</dt><dd className="mt-1 break-all font-mono text-xs text-gray-700">{detail.referenceId}</dd></div> : null}<div><dt className="text-gray-500">วันและเวลา</dt><dd className="mt-1 font-medium text-gray-900"><time dateTime={detail.time}>{detail.formattedTime}</time></dd></div>{detail.reason ? <div><dt className="text-gray-500">{detail.reasonLabel}</dt><dd className="mt-1 whitespace-pre-wrap font-medium text-gray-900">{detail.reason}</dd></div> : null}</dl>
