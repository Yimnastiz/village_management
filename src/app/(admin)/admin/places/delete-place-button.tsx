...
export function DeletePlaceButton({ placeId, placeName }: { placeId: string; placeName: string }) {
  ...
  const onDelete = async () => { setPending(true); try { const result = await adminDeleteVillagePlaceAction(placeId); if (!result.success) { toast.error(result.error); return; } setOpen(false); toast.success("ลบสถานที่เรียบร้อยแล้ว"); router.replace("/admin/places"); } catch { toast.error("ไม่สามารถลบสถานที่ได้ กรุณาลองใหม่อีกครั้ง"); } finally { setPending(false); } };
