...
export default async function Page({ searchParams }: PageProps) {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      <div className="max-h-none overflow-x-auto md:max-h-[60vh] md:overflow-auto"><table className="min-w-[560px] w-full text-sm">
        <thead className="sticky top-0 z-10 bg-gray-50 text-left text-gray-600 shadow-sm"><tr><th className="px-4 py-3">บ้านเลขที่</th><th className="px-4 py-3">จำนวนคน</th><th className="px-4 py-3">สมาชิกที่ผูก</th><th className="px-4 py-3">การจัดการ</th></tr></thead>
        <tbody>{houses.map((house) => <tr key={house.id} className="border-t border-gray-100"><td className="break-words px-4 py-3 font-medium text-gray-900">{house.houseNumber}</td><td className="px-4 py-3 text-gray-700">{house._count.persons.toLocaleString("th-TH")} คน</td><td className="px-4 py-3 text-gray-700">{house._count.memberships.toLocaleString("th-TH")} บัญชี</td><td className="px-4 py-3"><Link href={`/admin/population/houses/${house.id}`} className="inline-flex items-center justify-center rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">ดูรายละเอียด</Link></td></tr>)}</tbody>
      </table></div>
      {!houses.length ? <div className="px-4 py-10 text-center text-sm text-gray-500">{keyword ? "ไม่พบบ้านตามเงื่อนไขที่ค้นหา" : <><p className="font-medium text-gray-700">ยังไม่มีข้อมูลทะเบียนบ้าน</p><p className="mt-1">เพิ่มบ้านเลขที่เพื่อเริ่มจัดทำทะเบียนครัวเรือน</p></>}</div> : null}
    </div>
  </div>;
}
