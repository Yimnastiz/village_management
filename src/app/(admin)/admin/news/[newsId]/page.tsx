...
export default async function AdminNewsDetailPage({ params }: { params: Promise<{ newsId: string }> }) {
  ...
  const membership = await prisma.villageMembership.findFirst({ where: { userId: session.id, villageId: session.activeVillageId!, status: "ACTIVE" }, select: { villageId: true } }); if (!membership) redirect("/auth/login");
  const news = await prisma.news.findFirst({ where: { id: newsId, villageId: membership.villageId }, include: { author: { select: { name: true, memberships: { where: { villageId: membership.villageId, status: "ACTIVE" }, select: { role: true } } } } } }); if (!news) notFound();
