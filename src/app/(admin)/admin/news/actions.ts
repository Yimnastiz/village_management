    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          title: "คำขอลบข่าวได้รับการอนุมัติ",
          body: `“${String(payload.title ?? "")}” ถูกลบแล้ว`,
          metadata: {
            source: "NEWS",
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          userId: submission.requesterId,
          type: NotificationType.NEWS,
          title: "คำขอเพิ่มข่าวได้รับการอนุมัติ",
          body: `“${parsed.value.title}” ถูกเผยแพร่แล้ว`,
          metadata: {
            source: "NEWS",
            submissionId: submission.id,
            newsId: news.id,
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.notification.create({
      data: {
        ...
        userId: submission.requesterId,
        type: NotificationType.NEWS,
        title: "คำขอแก้ไขข่าวได้รับการอนุมัติ",
        body: `“${parsed.value.title}” ได้รับการอัปเดตแล้ว`,
        metadata: {
          source: "NEWS",
          submissionId: submission.id,
          newsId: target.id,
...
export async function adminRejectNewsSubmissionAction(
  ...
  await prisma.notification.create({
    data: {
      ...
      type: NotificationType.NEWS,
      title: "คำขอข่าวไม่ได้รับการอนุมัติ",
      body: `เหตุผล: ${normalizedReviewNote}`,
      metadata: {
        source: "NEWS",
        submissionId,
        status: "REJECTED",
