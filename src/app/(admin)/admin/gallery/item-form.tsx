...
type Props = { mode: "create" | "edit"; albumId: string; itemId?: string; hasExistingItems?: boolean; defaultValues?: { title: string; fileUrl: string; fileKey?: string; mimeType: string; sortOrder: string; isCover?: boolean } };
export function ItemForm({ mode, albumId, itemId, hasExistingItems = false, defaultValues }: Props) {
 const router = useRouter(); const [images, setImages] = useState<GalleryImageDraft[]>(defaultValues?.fileUrl ? [{ id: "existing", url: defaultValues.fileUrl, fileKey: defaultValues.fileKey, description: defaultValues.title || "", sortOrder: Number(defaultValues.sortOrder || 0), isCover: Boolean(defaultValues.isCover) }] : []);
 ...
 const submit = async (data: FormData) => { if (!images.length) { setError("root", { message: "กรุณาเพิ่มรูปภาพ" }); return; }
  const result = mode === "create" ? await createGalleryItemsAction(albumId, { items: images.map((image, index) => ({ fileUrl: image.url, fileKey: image.fileKey, uploadToken: image.uploadToken, title: image.description, isCover: image.isCover, sortOrder: index })) }) : await updateGalleryItemAction(albumId, itemId ?? "", { fileUrl: images[0].url, fileKey: images[0].fileKey, uploadToken: images[0].uploadToken, title: images[0].description || data.title, sortOrder: data.sortOrder, isCover: images[0].isCover });
