...
function getAppointmentSource(timeline: Array<{ action: string; metadata: Prisma.JsonValue | null; actor: { name: string | null; email: string | null; memberships: Array<{ role: VillageMembershipRole }> } | null }>) {
...
export default async function AdminAppointmentDetailPage({ params }: { params: Promise<{ appointmentId: string }> }) {
  ...
  const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, village: { memberships: { some: { userId: session.id, status: "ACTIVE", role: { in: ["HEADMAN", "ASSISTANT_HEADMAN", "COMMITTEE"] } } } } }, include: { user: { select: { name: true, email: true, phoneNumber: true } }, slot: true, timeline: { orderBy: { createdAt: "asc" }, take: 1, include: { actor: { select: { name: true, email: true, memberships: { where: { status: "ACTIVE" }, select: { role: true }, take: 1 } } } } } } });
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    ...
    <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="grid gap-3 border-t pt-4 text-sm sm:grid-cols-2">
        ...
        {source ? <p className="sm:col-span-2 text-gray-600">{source}</p> : null}
        {appointment.slot ? <p className="flex items-center gap-1.5 font-medium text-gray-900 sm:col-span-2"><Clock aria-hidden className="h-4 w-4 shrink-0 text-gray-500" />{isConfirmed ? "นัดหมาย" : "เสนอเวลา"}: {formatThaiDate(appointment.slot.date)} เวลา {appointment.slot.startTime}{appointment.slot.endTime ? ` - ${appointment.slot.endTime}` : ""}</p> : null}
    ...
    {["PENDING_APPROVAL", "TIME_SUGGESTED", "APPROVED"].includes(appointment.stage) ? <ProposeTimeForm appointmentId={appointment.id} title={appointment.title} description={appointment.description ?? ""} canReject={appointment.stage === "PENDING_APPROVAL"} canCancel /> : null}
  </div>;
}
