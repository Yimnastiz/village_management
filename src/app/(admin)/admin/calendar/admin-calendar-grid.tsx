...
export function AdminCalendarGrid({ year, monthIndex, todayKey, initialDate, searchKeyword, activeType, events, appointments }: { year: number; monthIndex: number; todayKey: string; initialDate: string | null; searchKeyword: string; activeType: "ALL" | "EVENT" | "APPOINTMENT"; events: EventItem[]; appointments: AppointmentItem[] }) {
  ...
  const visibleEvents = (activeType === "APPOINTMENT" ? [] : events).filter((item) => matches(`${item.title} ${item.location ?? ""}`));
  const visibleAppointments = (activeType === "EVENT" ? [] : appointments).filter((item) => matches(`${item.title} ${item.userName}`));
  ...
  return <>
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      <div className="grid grid-cols-7 border-b border-gray-200 bg-gray-50">{["เธญเธฒ", "เธ", "เธญ", "เธ", "เธเธค", "เธจ", "เธช"].map((day) => <div key={day} className="px-1 py-2 text-center text-xs font-semibold text-gray-600">{day}</div>)}</div>
      <div className="grid grid-cols-7">{Array.from({ length: blanks }).map((_, index) => <div key={`blank-${index}`} className="min-h-16 border-b border-r border-gray-100 bg-gray-50/60 sm:min-h-28 lg:min-h-32" />)}{Array.from({ length: days }).map((_, index) => { const day = index + 1, date = dateKey(day), dayEvents = eventsByDate.get(date) ?? [], dayAppointments = appointmentsByDate.get(date) ?? [], selected = selectedDate === date, today = todayKey === date; return <div key={date} className={`relative min-h-16 border-b border-r border-gray-100 sm:min-h-28 lg:min-h-32 ${selected ? "bg-green-50/70 ring-1 ring-inset ring-green-300" : "bg-white hover:bg-gray-50/80"} ${today && !selected ? "ring-1 ring-inset ring-rose-300" : ""}`}><button type="button" aria-pressed={selected} aria-current={selected ? "date" : undefined} aria-label={`เน€เธฅเธทเธญเธเธงเธฑเธเธ—เธตเน ${day}`} onClick={() => selectDate(date)} className="absolute inset-0 rounded-none focus-visible:z-20 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-green-700" /><div className="pointer-events-none absolute inset-x-2 top-2 z-10 flex items-center justify-between gap-1"><span className={`text-sm font-semibold ${today ? "text-rose-600" : "text-gray-800"}`}>{day}</span>{today ? <span className="rounded px-1 text-[10px] font-medium text-rose-700 sm:text-xs">เธงเธฑเธเธเธตเน</span> : null}</div><div className="pointer-events-none relative z-10 hidden space-y-1.5 px-2 pt-10 sm:block">{dayAppointments.length ? <CalendarItemRow title={dayAppointments[0].title} remaining={dayAppointments.length - 1} tone="appointment" href={`/admin/appointments/${dayAppointments[0].id}`} /> : null}{dayEvents.length ? <CalendarItemRow title={dayEvents[0].title} remaining={dayEvents.length - 1} tone="event" href={`/admin/calendar/${dayEvents[0].id}`} /> : null}</div>{dayAppointments.length + dayEvents.length ? <div className="pointer-events-none absolute bottom-2 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 sm:hidden">{dayAppointments.length ? <span className="h-1.5 w-1.5 rounded-full bg-sky-500" /> : null}{dayEvents.length ? <span className="h-1.5 w-1.5 rounded-full bg-green-600" /> : null}</div> : null}</div>; })}</div>
      <div className="border-t border-gray-200 bg-gray-50 px-3 py-2"><span className="inline-flex items-center gap-3 text-xs text-gray-600">{activeType !== "EVENT" ? <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-sky-500" />นัดหมาย</span> : null}{activeType !== "APPOINTMENT" ? <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-green-600" />กิจกรรม</span> : null}<span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-rose-500" />วันนี้</span></span></div>
    </section>
    {selectedDate ? <section ref={detailRef} className="scroll-mt-[calc(var(--app-sticky-top)+5rem)] rounded-xl border border-gray-200 bg-white p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><h2 className="text-base font-semibold text-gray-900 sm:text-lg">เธฃเธฒเธขเธฅเธฐเน€เธญเธตเธขเธ”เธงเธฑเธเธ—เธตเนเน€เธฅเธทเธญเธ: {new Date(`${selectedDate}T00:00:00`).toLocaleDateString("th-TH", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</h2><span className="shrink-0 pt-0.5 text-sm text-gray-500">{totalSelected} เธฃเธฒเธขเธเธฒเธฃ</span></div>{!totalSelected ? <p className="mt-5 text-sm text-gray-500">{activeType === "EVENT" ? "ไม่มีหกิจกรรมในวันนี้" : activeType === "APPOINTMENT" ? "ไม่มีนัดหมายในวันนี้" : "ไม่มีกิจกรรมหรือ นัดหมายในวันนี้"}</p> : <div className="mt-5 space-y-5">{activeType !== "EVENT" && selectedAppointments.length ? <section><h3 className="text-sm font-semibold text-gray-800">เธเธฑเธ”เธซเธกเธฒเธข ({selectedAppointments.length})</h3><div className="mt-2 divide-y divide-gray-100 border-y border-gray-100">{selectedAppointments.map((item) => <Link key={item.id} href={`/admin/appointments/${item.id}`} className="block py-3 hover:bg-gray-50"><p className="font-medium text-gray-900">{item.title}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><Clock className="h-4 w-4" aria-hidden="true" />{timeLabel(item.scheduledAt)} ยท {item.userName}</p></Link>)}</div></section> : null}{activeType !== "APPOINTMENT" && selectedEvents.length ? <section className={selectedAppointments.length ? "border-t border-gray-100 pt-5" : ""}><h3 className="text-sm font-semibold text-gray-800">เธเธดเธเธเธฃเธฃเธก ({selectedEvents.length})</h3><div className="mt-2 divide-y divide-gray-100 border-y border-gray-100">{selectedEvents.map((item) => <Link key={item.id} href={`/admin/calendar/${item.id}`} className="block py-3 hover:bg-gray-50"><p className="font-medium text-gray-900">{item.title}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><Clock className="h-4 w-4" aria-hidden="true" />{timeLabel(item.startsAt)}{item.endsAt ? ` - ${timeLabel(item.endsAt)}` : ""}</p>{item.location ? <p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><MapPin className="h-4 w-4" aria-hidden="true" />{item.location}</p> : null}</Link>)}</div></section> : null}</div>}</section> : null}
  </>;
}
