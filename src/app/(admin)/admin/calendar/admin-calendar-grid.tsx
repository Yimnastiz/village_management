...
export function AdminCalendarGrid({ year, monthIndex, todayKey, initialDate, searchKeyword, activeType, events, appointments }: { year: number; monthIndex: number; todayKey: string; initialDate: string | null; searchKeyword: string; activeType: "ALL" | "EVENT" | "APPOINTMENT"; events: EventItem[]; appointments: AppointmentItem[] }) {
  ...
  return <>
    ...
    {selectedDate ? <section ref={detailRef} className="scroll-mt-[calc(var(--app-sticky-top)+5rem)] rounded-xl border border-gray-200 bg-white p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><h2 className="text-base font-semibold text-gray-900 sm:text-lg">รายละเอียดวันที่เลือก: {new Date(`${selectedDate}T00:00:00`).toLocaleDateString("th-TH", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</h2><span className="shrink-0 pt-0.5 text-sm text-gray-500">{totalSelected} รายการ</span></div>{!totalSelected ? <p className="mt-5 text-sm text-gray-500">{activeType === "EVENT" ? "ไม่มีกิจกรรมในวันนี้" : activeType === "APPOINTMENT" ? "ไม่มีนัดหมายในวันนี้" : "ไม่มีกิจกรรมหรือนัดหมายในวันนี้"}</p> : <div className="mt-5 space-y-5">{activeType !== "EVENT" && selectedAppointments.length ? <section><h3 className="text-sm font-semibold text-gray-800">นัดหมาย ({selectedAppointments.length})</h3><div className="mt-2 divide-y divide-gray-100 border-y border-gray-100">{selectedAppointments.map((item) => <Link key={item.id} href={`/admin/appointments/${item.id}`} className="block py-3 hover:bg-gray-50"><p className="font-medium text-gray-900">{item.title}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><Clock className="h-4 w-4" aria-hidden="true" />{timeLabel(item.scheduledAt)} · {item.userName}</p></Link>)}</div></section> : null}{activeType !== "APPOINTMENT" && selectedEvents.length ? <section className={selectedAppointments.length ? "border-t border-gray-100 pt-5" : ""}><h3 className="text-sm font-semibold text-gray-800">กิจกรรม ({selectedEvents.length})</h3><div className="mt-2 divide-y divide-gray-100 border-y border-gray-100">{selectedEvents.map((item) => <Link key={item.id} href={`/admin/calendar/${item.id}`} className="block py-3 hover:bg-gray-50"><p className="font-medium text-gray-900">{item.title}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><Clock className="h-4 w-4" aria-hidden="true" />{timeLabel(item.startsAt)}{item.endsAt ? ` - ${timeLabel(item.endsAt)}` : ""}</p>{item.location ? <p className="mt-1 flex items-center gap-1.5 text-sm text-gray-500"><MapPin className="h-4 w-4" aria-hidden="true" />{item.location}</p> : null}</Link>)}</div></section> : null}</div>}</section> : null}
  </>;
}
