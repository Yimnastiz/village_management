...
import { categoryClassName, categoryLabel, feedbackBodyPreview, feedbackDisplayTitle } from "./feedback-presentation";
...
function href(values: { q: string; status: FeedbackStatusFilter; category: string; page?: number }) {
  ...
  if (values.status !== "active" && values.status !== "all") params.set("status", values.status);
...
export default async function AdminFeedbackPage({ searchParams }: Props) {
  ...
  const hasFilters = Boolean(q) || status !== "all" || category !== "all";
  const statusGroup: ToolbarGroup = { label: "สถานะ", options: [
    { label: "ทั้งหมด", href: href({ q, category, status: "all" }), active: status === "all" || status === "active", isDefault: true },
    { label: "ยังไม่ได้อ่าน", href: href({ q, category, status: NotificationStatus.UNREAD }), active: status === NotificationStatus.UNREAD },
    { label: "อ่านแล้ว", href: href({ q, category, status: NotificationStatus.READ }), active: status === NotificationStatus.READ },
    { label: "จัดเก็บ", href: href({ q, category, status: NotificationStatus.ARCHIVED }), active: status === NotificationStatus.ARCHIVED },
  ] };
  const categoryGroup: ToolbarGroup = { label: "ประเภท", options: [
    { label: "ทั้งหมด", href: href({ q, status, category: "all" }), active: category === "all", isDefault: true },
    { label: "ข้อเสนอแนะ", href: href({ q, status, category: "suggestion" }), active: category === "suggestion" },
    { label: "ข้อร้องเรียน", href: href({ q, status, category: "complaint" }), active: category === "complaint" },
    { label: "รายงานข้อผิดพลาด", href: href({ q, status, category: "bug" }), active: category === "bug" },
    { label: "อื่น ๆ", href: href({ q, status, category: "other" }), active: category === "other" },
  ] };

  return <div data-admin-compact-top className="space-y-3">
    <AdminPageToolbar sticky title={title} description="ตรวจสอบความคิดเห็นและข้อเสนอแนะจากผู้ใช้งาน" search={{ keyword: q, label: "ค้นหาความคิดเห็น", placeholder: "ค้นหาหัวข้อ รายละเอียด ชื่อ หรืออีเมล" }} filters={<><AdminFilterDropdown group={statusGroup} /><AdminFilterDropdown group={categoryGroup} /></>} activeFilterCount={(status === "all" || status === "active" ? 0 : 1) + (category === "all" ? 0 : 1)} />
    ...
    {data.rows.length ? <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm" aria-label="รายการความคิดเห็น"><div className="divide-y divide-gray-100">{data.rows.map((row) => {
      const name = feedbackMetadataString(row.metadata, "name"); const email = feedbackMetadataString(row.metadata, "email"); const categoryName = feedbackMetadataString(row.metadata, "category"); const unread = row.status === NotificationStatus.UNREAD; const archived = row.status === NotificationStatus.ARCHIVED; const displayTitle = feedbackDisplayTitle(row.title, row.body); const bodyPreview = feedbackBodyPreview(row.body);
      return <Link key={row.id} href={`/admin/feedback/${row.id}`} className={`group block px-4 py-4 transition-colors focus:outline-none focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-green-600 sm:px-5 ${unread ? "bg-emerald-50/60 hover:bg-emerald-50" : archived ? "bg-gray-50/70 text-gray-500 hover:bg-gray-100/70" : "bg-white hover:bg-gray-50/70"}`}><article><div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${categoryClassName(categoryName)}`}>{categoryLabel(categoryName)}</span>{unread ? <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-700"><span className="h-1.5 w-1.5 rounded-full bg-emerald-600" aria-hidden="true" />ยังไม่ได้อ่าน</span> : null}{archived ? <span className="text-xs text-gray-400">จัดเก็บแล้ว</span> : null}</div><h2 className={`mt-2 break-words text-base transition-colors group-hover:text-emerald-700 ${unread ? "font-semibold text-gray-950" : archived ? "font-medium text-gray-600" : "font-medium text-gray-900"}`}>{displayTitle}</h2></div><time className="shrink-0 text-xs text-gray-400 sm:pt-1" dateTime={row.createdAt.toISOString()}>{row.createdAt.toLocaleString("th-TH")}</time></div>{bodyPreview ? <p className="mt-1.5 line-clamp-2 whitespace-pre-wrap break-words text-sm leading-6 text-gray-600">{bodyPreview}</p> : null}<p className="mt-2 break-words text-xs text-gray-500">{name || "ไม่ระบุชื่อ"}{email ? <> <span aria-hidden="true">·</span> <span className="break-all">{email}</span></> : null}</p></article></Link>;
