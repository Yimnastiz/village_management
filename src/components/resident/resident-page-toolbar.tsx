  ...
  >
    ...
    {hasTools ? <div className={cn("flex min-w-0 flex-wrap items-center gap-2", !shouldHideHeading && "mt-2")} onKeyDown={(event) => { if (event.key === "Escape" && filterExpanded) closeFilter(); }}>
      ...
      {filters ? <>
        ...
        {filterExpanded ? <ResidentFilterDropdownContext.Provider value={{ openDropdown, setOpenDropdown, keepFiltersOpen }}><div id={filterPanelId} className="relative z-30 flex min-w-0 basis-full flex-wrap items-center gap-2 overflow-visible rounded-lg border border-gray-200 bg-white px-2 py-1.5 sm:basis-auto" onClickCapture={keepFiltersOpen} onChangeCapture={keepFiltersOpen}>{filters}</div></ResidentFilterDropdownContext.Provider> : <div id={filterPanelId} hidden />}
      </> : null}
