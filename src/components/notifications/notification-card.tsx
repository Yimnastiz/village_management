...
export function NotificationCard({ notification, title, body, destination, markAsRead }: Props) {
  ...
  return (
    <article className={`overflow-hidden rounded-xl border transition-colors ${presentation.accentClassName ?? ""} ${isRead ? "border-gray-200 bg-white" : "border-blue-200 bg-blue-50/70"}`}>
      <button type="button" onClick={handleClick} disabled={isUpdating}
        ...
        <span className="min-w-0 flex-1">
          ...
          {body ? <span className={`mt-1 block break-words text-sm leading-6 text-gray-600 ${presentation.bodyClassName ?? (presentation.sourceAttribution ? "line-clamp-2 whitespace-normal" : "")}`}>{body}</span> : null}
