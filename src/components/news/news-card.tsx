...
type NewsCardProps = { href: string; title: string; summary?: string | null; imageUrl?: string | null; isPinned?: boolean; badge?: React.ReactNode; meta: string };

export function NewsCard({ href, title, summary, imageUrl, isPinned, badge, meta }: NewsCardProps) {
  return <Link href={href} className="group block overflow-hidden rounded-xl border border-gray-200 bg-white transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
    <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-100">
      ...
      {isPinned ? <span title="ข่าวปักหมุด" aria-label="ข่าวปักหมุด" className="absolute right-3 top-3 rounded-full bg-white/90 p-2 text-amber-600 shadow-sm"><Pin className="h-4 w-4" /></span> : null}
    </div>
    <div className="p-4"><div className="mb-2 flex items-center gap-2">{badge}</div><h2 className="line-clamp-2 font-semibold leading-6 text-gray-900 group-hover:text-green-700">{title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-gray-500">{summary?.trim() || "อ่านรายละเอียดและความคืบหน้าจากหมู่บ้าน"}</p><p className="mt-3 text-xs text-gray-400">{meta}</p></div>
  </Link>;
}
