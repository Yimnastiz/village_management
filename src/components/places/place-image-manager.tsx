...
type Props = {
  ...
  helpText?: string;
  /** Optional read-only, per-image context rendered within an image card. */
  renderItemMetadata?: (image: PlaceImageView) => ReactNode;
};

const makeId = () => typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `place-${Date.now()}-${Math.random()}`;

function SortableImage({ item, index, total, onCover, onRemove, onRetry, onMove, onDescription, allowCoverSelection, allowReorder, renderItemMetadata }: {
  item: Item; index: number; total: number; onCover: () => void; onRemove: () => void; onRetry: () => void; onMove: (offset: number) => void; onDescription: (description: string) => void; allowCoverSelection: boolean; allowReorder: boolean; renderItemMetadata?: (image: PlaceImageView) => ReactNode;
}) {
...
export function PlaceImageManager({ value, onChange, onBusyChange, disabled, maxCount = MAX_IMAGES_PER_REQUEST, label = "รูปภาพ", autoSelectFirstCover = true, allowCoverSelection = true, allowReorder = true, maxSizeBytes = MAX_IMAGE_BYTES, uploadEndpoint = "/api/places/images", helpText, renderItemMetadata }: Props) {
