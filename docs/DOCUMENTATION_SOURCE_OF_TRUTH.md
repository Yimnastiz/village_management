...
Phase 4A removed the Super Admin route, API, access-code, cookie, and bootstrap runtime. Phase 4B removed remaining active `SUPERADMIN` and `ASSISTANT_HEADMAN` runtime semantics: active authority and current administrative classification now derive solely from an active Headman membership. Historical audit, notification, broadcast, and issue-message presentation continues to recognize legacy metadata. Prisma's `SystemRole.SUPERADMIN`, `VillageMembershipRole.ASSISTANT_HEADMAN`, and legacy source values remain temporarily for the later Phase 4C schema/data migration; they grant no runtime authority.
