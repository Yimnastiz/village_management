...
`/admin/security` is the intended final Village Audit workspace. It is scoped to the current Village and supports all-activity and explicit important-record views; legacy Assistant and Super Admin audit facts remain readable during transition. Headman manages Village Settings at `/admin/settings/village`: Village name, description, address, phone, email, and website are editable; Moo, geography, and slug remain read-only to protect configured geographic/catalog identity and public URLs. Transitional Super Admin routes remain until Phase 4.
